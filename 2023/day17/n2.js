...
const minimizeHeatLoss = () => {
  ...
  buckets[input[0][0]].push([0, 0, 0, "start", 0]);
  ...
  while (currentDistance <= maxDistance) {
    ...
    while (bucket.length > 0) {
      ...
      if (x === cols - 1 && y === rows - 1 && stepsInSameDirection >= 4) {
        return distance;
      }
      ...
      directions
        ...
        .forEach(([dy, dx, newDirection]) => {
          ...
          if (
            ...
            return;
          if (newStepsInSameDirection > 10) return;
          ...
          if (newDistance > dist[newY][newX][i][newStepsInSameDirection])
            return;
