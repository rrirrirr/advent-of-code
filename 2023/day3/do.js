    ...
    (acc, c, i) => {
      ...
      if (s) {
        if (!isNumber || i === line.length - 1) {
          r[r.length - 1].push(i === line.length - 1 && isNumber ? i : i - 1);
          return { s: false, r };
        }
        return acc;
      }
...
const isAdjacentToSymbol = ([y, start, end]) => {
  return [...Array(end - start + 1).keys()]
    .map((i) => start + i)
    .some((x) => {
      const adjacentPositions = [
        [Math.max(y - 1, 0), Math.max(x - 1, 0)], // Upper-left
        [Math.max(y - 1, 0), x], // Up
        [Math.max(y - 1, 0), Math.min(x + 1, input[0].length - 1)], // Upper-right
        [y, Math.max(x - 1, 0)], // Left
        [y, Math.min(x + 1, input[0].length - 1)], // Right
        [Math.min(y + 1, input.length - 1), Math.max(x - 1, 0)], // Lower-left
        [Math.min(y + 1, input.length - 1), x], // Down
        [
          Math.min(y + 1, input.length - 1),
          Math.min(x + 1, input[0].length - 1),
        ], // Lower-right
      ];

      return adjacentPositions.some(([adjY, adjX]) => {
        return isSymbol(input[adjY][adjX]);
      });
    });
};
...
const toNumbers = ([y, start, end]) => {
  const num = [...Array(end - start + 1).keys()]
    .map((i) => input[y][start + i])
    .join("");
  return +num;
};

const findGears = (line, y) => {
  return line.map((c, x) => [y, x, c]).filter(([y, x, c]) => c === "*");
};

const adjacentNumbers = ([y, x], u) => {
  const adjacentPositions = [
    [Math.max(y - 1, 0), Math.max(x - 1, 0)], // Upper-left
    [Math.max(y - 1, 0), x], // Up
    [Math.max(y - 1, 0), Math.min(x + 1, input[0].length - 1)], // Upper-right
    ...
    [Math.min(y + 1, input.length - 1), Math.max(x - 1, 0)], // Lower-left
    [Math.min(y + 1, input.length - 1), x], // Down
    [Math.min(y + 1, input.length - 1), Math.min(x + 1, input[0].length - 1)], // Lower-right
  ];

  return adjacentPositions.reduce((acc, [adjY, adjX]) => {
    if (isNumber(u[adjY][adjX])) {
      return [...acc, u[adjY][adjX]];
    }
...
const removeDuplicates = (indexes) => {
  const pickedIndex = {};
  return indexes.filter((index) => {
    if (index in pickedIndex) return false;
    pickedIndex[index] = true;
    return true;
  });
};

const hasTwoAdjacent = (indexes) => {
  return indexes.length === 2;
};

const findGearPower = ([i1, i2], powerIndex) => {
  return powerIndex[i1] * powerIndex[i2];
};

// 1
// const result = input
//   .flatMap((line, y) => findNumbers(line, y))
...
const powerIndex = numbers.reduce((acc, [y, start, end, i]) => {
  return { ...acc, [i]: toNumbers([y, start, end]) };
}, {});

const update = input;
numbers.forEach(([y, start, end, id]) => {
...
const result = input
  .flatMap((line, y) => findGears(line, y))
  .map(([y, x]) => adjacentNumbers([y, x], update))
  .map(removeDuplicates)
  .filter(hasTwoAdjacent)
  .map((c) => findGearPower(c, powerIndex))
  .reduce((acc, n) => acc + n, 0);
////////
// console.log(update);
// console.log(numbers.map(toNumbers));
console.log(result);
